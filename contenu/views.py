...
def page_serie(request, id):
    ...
    #In case there is no episode in the selected serie
    try:
        saison = [x for x in range(1, (episode.latest('saison').saison)+1)]
    except AttributeError:
        return redirect('gerant_accueil')
    return render(request, 'page_serie.html', {'serie':serie , 'episodes':episode,  'saison':saison, 'ref':ref, 'ref_id':ref_id })
...
def watchlist(request):
    ...
    if cas in request.user.anime_prefere.all():
